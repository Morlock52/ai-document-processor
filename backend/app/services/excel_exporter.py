...
class ExcelExporter:
    ...
    def _add_completeness_chart(self, ws, data: List[Dict[str, Any]]):
        ...
        for field, stats in sorted(field_completeness.items())[:10]:  # Top 10 fields
            completeness = (
                (stats["filled"] / stats["total"] * 100) if stats["total"] > 0 else 0
            )
            ws.append([field, completeness])
        ...
        data_ref = Reference(
            ws, min_col=2, min_row=chart_start_row, max_row=ws.max_row, max_col=2
        )
        cats = Reference(ws, min_col=1, min_row=chart_start_row + 1, max_row=ws.max_row)
    ...
    async def export_batch(
        self, documents: List[Dict[str, Any]], format: str = "xlsx"
    ) -> bytes:
    ...
    async def export_template(
        self, documents: List[Dict[str, Any]], field_names: List[str]
    ) -> bytes:
    ...
    def _write_template_info_sheet(
        self, ws, documents: List[Dict[str, Any]], field_names: List[str]
    ):
