...
async def upload_document(
    ...
    logger.info(
        f"📤 UPLOAD_START: File='{file.filename}', Size={file.size}B, Client={client_ip}"
    )
    ...
    try:
        ...
        if not file.filename:
            logger.warning(
                f"❌ UPLOAD_FAILED: No filename provided, Client={client_ip}"
            )
            raise HTTPException(status_code=400, detail="Filename is required")

        if not file.filename.lower().endswith(".pdf"):
            logger.warning(
                f"❌ UPLOAD_FAILED: Invalid file type '{file.filename}', Client={client_ip}"
            )
        ...
        if file.size and file.size > settings.MAX_UPLOAD_SIZE:
            logger.warning(
                f"❌ UPLOAD_FAILED: File too large {file.size}B > {settings.MAX_UPLOAD_SIZE}B, Client={client_ip}"
            )
            raise HTTPException(
                status_code=400,
        ...
        logger.info(
            f"📝 UPLOAD_PROCESSING: Generated filename='{unique_filename}', Hash={file_hash[:8]}..."
        )
        ...
        try:
            ...
            logger.debug(
                f"📁 UPLOAD_DIR: Ensured directory exists: {settings.UPLOAD_DIR}"
            )
        except OSError as e:
        ...
        try:
            async with aiofiles.open(local_path, "wb") as f:
                ...
                if not content:
                    logger.warning(
                        f"❌ UPLOAD_FAILED: Empty file content, Client={client_ip}"
                    )
                    raise HTTPException(
                ...
                logger.info(
                    f"💾 FILE_SAVED: Local path='{local_path}', Size={len(content)}B"
                )
        ...
        try:
            ...
            logger.info(
                f"💾 DB_CREATED: Document ID={document.id}, Status={document.status}"
            )
        ...
        if settings.AWS_ACCESS_KEY_ID:
            try:
                ...
                logger.info(
                    f"☁️  S3_UPLOAD_SUCCESS: Key='{s3_key}', Document ID={document.id}"
                )

            except Exception as e:
                # Log S3 error but don't fail the upload
                logger.warning(
                    f"⚠️  S3_UPLOAD_FAILED: {e}, Document ID={document.id} (continuing without S3)"
                )

        # Success logging with timing
        elapsed_time = time.time() - start_time
        logger.info(
            f"✅ UPLOAD_SUCCESS: Document ID={document.id}, Time={elapsed_time:.2f}s, Client={client_ip}"
        )
        ...
        elapsed_time = time.time() - start_time
        logger.error(
            f"❌ UPLOAD_ERROR: Unexpected error after {elapsed_time:.2f}s: {e}"
        )
...
async def process_document(
    ...
    try:
        ...
        if not document:
            logger.warning(
                f"❌ PROCESS_FAILED: Document ID={document_id} not found, Client={client_ip}"
            )
            raise HTTPException(status_code=404, detail="Document not found")

        logger.info(
            f"📄 DOCUMENT_FOUND: ID={document_id}, Status={document.status}, File='{document.original_filename}'"
        )
        ...
        if document.filename:
            ...
            if not os.path.exists(local_path):
                logger.error(
                    f"❌ FILE_MISSING: Document ID={document_id}, Path='{local_path}'"
                )
                ...
                db.commit()
                raise HTTPException(
                    status_code=400, detail="Document file not found on server"
                )

        # Check if document is already being processed
        if document.status == ProcessingStatus.PROCESSING:
            logger.info(
                f"⏳ ALREADY_PROCESSING: Document ID={document_id}, Progress={document.progress}"
            )
            return JSONResponse(
                status_code=200,
                content={
        ...
        logger.info(
            f"⚙️  PROCESS_OPTIONS: Schema={bool(schema)}, Template={template_mode}, Priority={priority}"
        )
        ...
        try:
            ...
            job_id = enqueue_document_processing(document_id=document_id, schema=schema)

            logger.info(
                f"📋 QUEUE_SUCCESS: Job ID='{job_id}' for document ID={document_id}"
            )

        except Exception as e:
            logger.error(
                f"❌ QUEUE_FAILED: Error enqueueing document ID={document_id}: {e}"
            )
            raise HTTPException(
                status_code=500, detail=f"Failed to queue document processing: {str(e)}"
            )
        ...
        try:
            ...
            logger.info(
                f"💾 STATUS_UPDATED: Document ID={document_id}, Status={document.status}, Job='{job_id}'"
            )
        ...
        logger.info(
            f"✅ PROCESS_QUEUED: Document ID={document_id}, Job='{job_id}', Time={elapsed_time:.2f}s"
        )
        ...
        elapsed_time = time.time() - start_time
        logger.error(
            f"❌ PROCESS_ERROR: Document ID={document_id}, Time={elapsed_time:.2f}s, Error: {e}"
        )
        ...
        try:
            if "document" in locals() and document:
                ...
                db.commit()
                logger.info(
                    f"🔄 STATUS_RECOVERY: Marked document ID={document_id} as failed"
                )
        except Exception as recovery_error:
            logger.error(
                f"❌ RECOVERY_FAILED: Could not update document status: {recovery_error}"
            )
            db.rollback()

        raise HTTPException(
            status_code=500, detail="Internal error during process initialization"
        )
...
async def download_template_excel(
    ...
    logger.info(
        f"📊 TEMPLATE_START: Documents={document_ids}, Count={len(document_ids)}, Client={client_ip}"
    )
    ...
    try:
        ...
        if not document_ids:
            logger.warning(
                f"❌ TEMPLATE_FAILED: No document IDs provided, Client={client_ip}"
            )
            raise HTTPException(
        ...
        if len(document_ids) > 100:  # Reasonable limit
            logger.warning(
                f"❌ TEMPLATE_FAILED: Too many documents {len(document_ids)}, Client={client_ip}"
            )
            raise HTTPException(
                status_code=400, detail="Maximum 100 documents allowed per template"
            )

        logger.info(
            f"🔍 DB_QUERY: Searching for {len(document_ids)} completed documents"
        )
        ...
        try:
            ...
            logger.info(
                f"📄 DOCUMENTS_FOUND: Found={len(documents)}, Missing={len(missing_ids)}"
            )
            ...
            raise HTTPException(
                status_code=500, detail="Database error while retrieving documents"
            )

        if not documents:
            logger.warning(
                f"❌ TEMPLATE_FAILED: No completed documents found for IDs={document_ids}"
            )
        ...
        for doc in documents:
            if doc.extracted_data:
                docs_with_data += 1
                logger.debug(
                    f"📊 PROCESSING_DOC: ID={doc.id}, Fields={len(doc.extracted_data)}"
                )

                # Extract field names from the document (skip metadata fields)
                doc_fields = [
                    key for key in doc.extracted_data.keys() if not key.startswith("_")
                ]
                ...
                docs_without_data += 1
                logger.warning(
                    f"⚠️  NO_DATA: Document ID={doc.id} has no extracted data"
                )

        logger.info(
            f"📊 DATA_SUMMARY: WithData={docs_with_data}, WithoutData={docs_without_data}, Fields={len(all_field_names)}"
        )

        if not template_data:
            logger.warning(
                "❌ TEMPLATE_FAILED: No extracted data available for template"
            )
            raise HTTPException(
                status_code=400, detail="No extracted data available for template"
            )

        if not all_field_names:
            logger.warning("❌ TEMPLATE_FAILED: No field names found in extracted data")
            raise HTTPException(
                status_code=400, detail="No field names found in documents"
            )

        # Export to Excel with comprehensive error handling
        try:
            logger.info(
                f"📊 EXCEL_EXPORT_START: Rows={len(template_data)}, Columns={len(all_field_names)}"
            )

            exporter = ExcelExporter()
            excel_data = await exporter.export_template(
                template_data, sorted(all_field_names)
            )

            if not excel_data:
                logger.error("❌ EXCEL_EXPORT_FAILED: Empty Excel data returned")
                raise HTTPException(
        ...
        elapsed_time = time.time() - start_time
        logger.info(
            f"✅ TEMPLATE_SUCCESS: File='{filename}', Time={elapsed_time:.2f}s, Size={len(excel_data)}B"
        )
        logger.info(
            f"📊 TEMPLATE_METRICS: Documents={len(documents)}, Fields={len(all_field_names)}, Rows={len(template_data)}"
        )
        ...
        elapsed_time = time.time() - start_time
        logger.error(
            f"❌ TEMPLATE_ERROR: Unexpected error after {elapsed_time:.2f}s: {e}"
        )
        logger.error(f"🔍 TEMPLATE_TRACEBACK: {traceback.format_exc()}")
        raise HTTPException(
            status_code=500, detail="Internal error during template generation"
        )
...
async def download_excel(
    ...
    excel_data = await exporter.export_to_excel(
        [document.extracted_data], include_metadata=include_metadata
    )
...
async def batch_process_documents(
    ...
    for doc in documents:
        if doc.status != ProcessingStatus.PROCESSING:
            try:
                job_id = enqueue_document_processing(document_id=doc.id, schema=schema)
    ...
    return {
        ...
        "job_ids": job_ids,
        "document_ids": [
            doc.id for doc in documents if doc.status == ProcessingStatus.PROCESSING
        ],
    }
...
async def batch_download_excel(
    ...
    documents = (
        db.query(Document)
        .filter(
            Document.id.in_(document_ids), Document.status == ProcessingStatus.COMPLETED
        )
        .all()
    )
